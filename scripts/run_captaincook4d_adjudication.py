...
def main() -> None:
    ...
    p.add_argument("--neighbor-frames", dest="neighbor_frames", action="store_true", default=True,
                   help="add prev-step-end / next-step-start frames (temporal context)")
    p.add_argument("--no-neighbor-frames", dest="neighbor_frames", action="store_false")
    p.add_argument("--abstain", dest="abstain", action="store_true", default=True,
                   help="allow the model to answer 'Insufficient Evidence'")
    p.add_argument("--no-abstain", dest="abstain", action="store_false")
    p.add_argument("--context-window", type=int, default=2,
                   help="recipe steps before/after the flagged step to include")
    ...
    print(f"neighbor_frames={args.neighbor_frames} abstain={args.abstain} "
          f"context_window={args.context_window} prompt={args.prompt}", flush=True)

    backend = (MockVLMBackend() if args.backend == "mock"
               else QwenVLBackend(model_id=args.model_id))
    builder_cls = ChainOfThoughtPromptBuilder if args.prompt == "cot" else ZeroShotPromptBuilder
    prompt_builder = builder_cls(allow_abstention=args.abstain)
    frame_provider = CaptainCook4DFrameProvider(
        args.data_root, args.frame_cache,
        segments=(segments if args.neighbor_frames else None))
    pipe = AdjudicationPipeline(
        trigger=AllEventsTrigger(),
        evidence_builder=DefaultEvidenceBuilder(
            frame_provider=frame_provider,
            context=CaptainCook4DProcedureContext(segments, window=args.context_window)),
    ...
    print(f"  n={scores.n} coverage={scores.coverage} family_acc={scores.family_accuracy} "
          f"committed_acc={scores.committed_family_accuracy} "
    ...
    # Fabrication metric: on events whose GT is an execution-cluster error
    # (Measurement/Technique/Preparation, ~90% of the set), a *committed*
    # out-of-cluster prediction (Timing/Temperature/Order/Missing/Other) is an
    # invented error the frames can't support. Abstentions do NOT count as
    # fabrications — that is the whole point of the escape hatch.
    CLUSTER = {"Measurement Error", "Technique Error", "Preparation Error"}
    gt_exec = [r for r in ok if normalize_family(gt[r.event_id]["family"]) in CLUSTER]
    fabrications = [r for r in gt_exec
                    if not r.attribution.abstained
                    and canonical_family(r.attribution) not in CLUSTER]
    n_abstain = sum(1 for r in ok if r.attribution.abstained)
    fab_rate = len(fabrications) / len(gt_exec) if gt_exec else 0.0
    print(f"\n  abstentions: {n_abstain}/{len(ok)}")
    print(f"  fabrications (committed out-of-cluster on GT-execution events): "
          f"{len(fabrications)}/{len(gt_exec)} = {fab_rate:.2f}", flush=True)
    if args.out:
